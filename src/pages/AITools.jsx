...
const AITools = () => {
    ...
    const MarkdownComponents = {
        h1: ({node, ...props}) => <h1 className="text-2xl font-bold mt-6 mb-3 text-slate-900 uppercase tracking-tight" {...props} />,
        h2: ({node, ...props}) => <h2 className="text-xl font-bold mt-5 mb-3 text-slate-900 uppercase" {...props} />,
        h3: ({node, ...props}) => <h3 className="text-lg font-bold mt-4 mb-2 text-slate-700" {...props} />,
        p: ({node, ...props}) => <p className="mb-4 leading-relaxed" {...props} />,
        ul: ({node, ...props}) => <ul className="list-disc pl-6 mb-4 space-y-1.5 marker:text-slate-600" {...props} />,
        ol: ({node, ...props}) => <ol className="list-decimal pl-6 mb-4 space-y-1.5 marker:text-slate-600 font-bold text-slate-900" {...props} />,
        li: ({node, ...props}) => <li className="text-slate-700 font-medium mb-1.5" {...props} />,
        strong: ({node, ...props}) => <strong className="font-bold text-slate-900" {...props} />,
        blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-slate-300 bg-slate-100/50 pl-5 py-2 pr-4 italic text-slate-600 my-4 rounded-r-xl shadow-inner" {...props} />,
        code: ({node, inline, ...props}) => 
            inline 
                ? <code className="bg-slate-100 text-blue-600 px-1.5 py-0.5 rounded text-sm font-mono border border-slate-200" {...props} />
                : <pre className="bg-slate-900 p-4 rounded-xl overflow-x-auto text-sm font-mono text-zinc-100 border border-slate-700 my-4 shadow-2xl"><code {...props} /></pre>
    };
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pb-24">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 px-1">
                <div className="flex items-center gap-4">
                    <h3 className="text-2xl font-bold text-slate-900 tracking-tight  uppercase">STUDY ASSISTANT</h3>
                    <div className="h-4 w-px bg-slate-200 hidden md:block" />
                    <p className="text-sm font-bold text-slate-600 uppercase tracking-wider hidden md:block leading-none mt-1">AI-Powered Study Tools</p>
                </div>
            </div>

            {/* Landscape Mode Tool Selector */}
            <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="glass-card p-2 md:p-3 border-slate-200 bg-white relative overflow-hidden shadow-2xl">
                <div className="flex flex-col lg:flex-row items-center gap-3 relative z-10">
                    <div className="flex-1 flex gap-2">
                        {tools.map(tool => (
                            ...
                            >
                                <tool.icon className={`w-3.5 h-3.5 ${activeTool === tool.id ? 'text-slate-700' : 'text-slate-700'}`} />
                                <span className="text-sm tracking-wider leading-none">{tool.name}</span>
                            </button>
                        ))}
                    </div>
                        ...
                        {loading ? (
                            ...
                            >
                                ...
                                <div className="space-y-2 text-center">
                                    <p className="font-bold text-slate-900 uppercase tracking-tight text-2xl animate-pulse">Generating Content</p>
                                    <p className="text-sm font-bold uppercase text-slate-600 tracking-[0.2em]">Processing your request for {topic}...</p>
                                </div>
                            </motion.div>
                        ) : currentChat ? (
                            <motion.div 
                            ...
                            >
                                <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-slate-100 backdrop-blur-md relative z-10 shrink-0">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-600 italic pb-0.5">AI Output</h4>
                                            <h5 className="text-lg font-bold text-slate-900 uppercase tracking-tight leading-none">{currentChat.topic}</h5>
                                        </div>
                                    </div>
                                    <motion.button 
                                    ...
                                    <div className="p-8">
                                        <div className="space-y-0">
                                            ...
                                            {currentChat.messages && currentChat.messages.map((msg, idx) => (
                                                <div key={idx} className={`flex gap-4 py-8 ${msg.role === 'model' ? 'border-b border-slate-200 bg-white/30 -mx-8 px-8' : ''}`}>
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-1 ${msg.role === 'user' ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-100 border border-slate-300 text-slate-700 shadow-inner'}`}>
                                                        {msg.role === 'user' ? <Terminal className="w-4 h-4" /> : <Bot className="w-5 h-5" />}
                                                    </div>
                                                    <div className="grow overflow-hidden">
                                                        <div className={`font-bold text-sm mb-2 uppercase tracking-wider ${msg.role === 'user' ? 'text-slate-600' : 'text-slate-600/80 flex items-center gap-2'}`}>
                                                            {msg.role === 'user' ? 'You' : <>Study AI <Sparkles className="w-3 h-3"/></>}
                                                        </div>
                                                        <div className="font-medium text-slate-700 text-sm max-w-none">
                                                            {msg.role === 'user' ? (
                                                                <div className="text-base text-slate-900 whitespace-pre-wrap">{msg.content}</div>
                                                            ) : (
                                                                <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]} components={MarkdownComponents}>
                                                                    {msg.content}
                                                                </ReactMarkdown>
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <p className="font-bold text-slate-900 uppercase tracking-tight text-2xl">Result View</p>
                                    <p className="text-sm font-bold uppercase tracking-wider text-slate-600">Output will appear here</p>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                ...
                <div className="xl:col-span-4 h-full min-h-[400px]">
                    <div className="glass-card flex flex-col overflow-hidden h-full border-slate-200 bg-white shadow-2xl">
                        <div className="p-6 border-b border-slate-200 flex items-center gap-3 bg-slate-100 backdrop-blur-md shrink-0">
                            ...
                            <h4 className="text-sm font-bold uppercase tracking-[0.2em] text-slate-300">Recent Chats</h4>
                        </div>
                            ...
                            {recentChats.length > 0 ? recentChats.map((chat) => (
                                ...
                                >
                                    ...
                                    <div className="overflow-hidden grow">
                                        <h5 className="text-sm font-bold text-slate-900 truncate">{chat.topic}</h5>
                                        <div className="flex gap-2 items-center mt-1">
                                            <span className="text-xs font-bold uppercase tracking-wider text-slate-700 bg-slate-100/50 px-1.5 py-0.5 rounded">{tools.find(t => t.id === chat.tool)?.name || chat.tool}</span>
                                            {chat.messages && chat.messages.length > 0 && (
                                                <span className="text-xs font-bold uppercase tracking-wider text-slate-600">{(chat.messages.length / 2).toFixed(0)} Follow-ups</span>
                                            )}
                                        </div>
                                    </div>
                                    <button 
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-slate-500 hidden0 space-y-4">
                                    ...
                                    <p className="text-sm font-bold uppercase tracking-wider">No recent history</p>
                                </div>
                            )}
                        </div>
                    </div>
